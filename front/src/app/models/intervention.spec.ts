...
import { InterventionsPage } from '../pages/interventions/interventions.page';
...
describe('InterventionsPage', () => {
  ...
  describe('ionViewWillEnter()', () => {
    it('should categorize interventions correctly by date and status', async () => {
      ...
      const testInterventions = [
        { ...mockInterventions[0], appointment_end: futureDate, status: 'scheduled' },
        { ...mockInterventions[1], appointment_end: pastDate, status: 'completed' }
      ] as any[];

      mockInterventionService.getInterventionsByUser.and.returnValue(testInterventions);

      await component.ionViewWillEnter();

      expect(component.pastInterventions.length).toBe(1);
      expect(component.upcomingInterventions.length).toBe(1);
    });
  });

  describe('confirmCancel()', () => {
    it('should call cancelIntervention', () => {
      spyOn(component, 'cancelIntervention');
      const intervention = mockInterventions[0];

      component.confirmCancel(intervention);

      expect(component.cancelIntervention).toHaveBeenCalledWith(intervention);
    });
  });
